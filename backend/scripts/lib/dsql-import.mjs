...
// O Aurora DSQL usa controle de concorrência otimista: quando duas transações
// tocam nas mesmas linhas ao mesmo tempo, uma delas recebe o erro 40001
// ("change conflicts with another transaction" / OC000). Isso não é um bug
// nos dados, é esperado — o jeito certo de lidar é tentar de novo com backoff.
function isConflitoTransacao(err) {
...
const podeRetentar = isConflitoTransacao(err) && tentativa < MAX_TENTATIVAS;
if (!podeRetentar) throw err;
const espera = 300 * tentativa + Math.floor(Math.random() * 300);
console.warn(
`[liberacao] conflito de transação (tentativa ${tentativa}/${MAX_TENTATIVAS}), tentando de novo em ${espera}ms`
);
