 ...
 * 2. Em Propriedades do script, adicione GEMINI_API_KEY com a chave da API Gemini.
...
function doPost(e) {
  try {
    ...
    const texto = chamarGemini_(key, prompt);
    if (!texto) return jsonOut({ ok: false, erro: "Gemini não retornou texto." });

    return jsonOut({ ok: true, texto: texto });
  } catch (err) {
